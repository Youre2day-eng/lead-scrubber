...
// localStorage fallback. Same conceptual path as Firestore so a future
// HIPAA migration can copy this verbatim into a real Firestore collection.
function localKey(uid: string) {
    return `leadscrubber:artifacts:${APP_ID}:users:${uid}:saved_leads`;
}
...
function writeLocal(uid: string, leads: SavedLead[]) {
    try {
          localStorage.setItem(localKey(uid), JSON.stringify(leads));
    } catch {
          // ignore
    }
}
...
export function useLeads(user: User | null) {
  ...
  // Persist a single saved lead in either backend.
  ...
  const deleteLead = async (leadId: string) => {
        if (!user) return;
        await removeLead(user.uid, leadId);
  };
  ...
  const sendMessage = async (lead: SavedLead, text: string): Promise<SavedLead> => {
        ...
        await patchLead(user.uid, lead.id, {
                stage: newStage,
                messages,
                lastContacted: new Date().toISOString(),
        } as Partial<SavedLead>);
