...
// Detect platform from a URL string. Used for grouping the source toggles.
...
export function ScraperView({ targetUrls, onGoToSettings, onSaveLead, sessionSavedIds, onNicheChange }: Props) {
  ...
  // Per-scan source overrides — Map of url-id -> enabled boolean.
  // null/undefined = use the saved setting from targetUrls.
  ...
  // Effective enabled state for each url: override if set, else saved enabled.
  const isUrlOn = (u: TargetUrl): boolean => {
    if (sourceOverrides[u.id] !== undefined) return sourceOverrides[u.id];
    return u.enabled !== false;
  };

  const enabledUrls = useMemo(() => targetUrls.filter(isUrlOn), [targetUrls, sourceOverrides]);

  // Group urls by platform for the toggle UI.
  ...
  const toggleUrl = (u: TargetUrl) => {
    setSourceOverrides(prev => ({ ...prev, [u.id]: !isUrlOn(u) }));
  };
  ...
  const togglePlatform = (platform: string, on: boolean) => {
    ...
    setSourceOverrides(prev => {
      const next = { ...prev };
      for (const u of urls) next[u.id] = on;
      return next;
    });
  };
  ...
  // Apply location filter (case-insensitive substring) on top of intent filter.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Mix location into the keyword list so server-side scoring picks it up too.
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Austin, TX or Denver, NYC"
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button type="button" onClick={onGoToSettings} className="text-xs text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
            ...
            {targetUrls.length === 0 ? (
              <div className="border border-amber-200 bg-amber-50 rounded-lg p-3 text-xs text-amber-800">
                No sources configured. Click <strong>Manage</strong> to add some.
              </div>
            ) : (
            ...
            <p className="text-xs text-slate-500 mt-1.5">
              {enabledUrls.length} of {targetUrls.length} sources will be scanned.
            </p>
          </div>

          <button
            type="submit"
            disabled={isRunning || !niche.trim() || enabledUrls.length === 0}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 transition"
          >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
              <h3 className="font-semibold text-slate-900">
                {filteredLeads.length} {filter === 'buying' ? 'Buyer' : filter === 'selling' ? 'Seller' : 'Lead'}{filteredLeads.length === 1 ? '' : 's'}
                {location.trim() && <span className="text-xs font-normal text-slate-500 ml-2">in "{location.trim()}"</span>}
              </h3>
            ...
            <div className="flex items-center gap-2 mb-4 border-b border-slate-100 pb-3 flex-wrap">
              ...
              <span className="text-xs text-slate-400 ml-auto">
                {filter === 'buying' ? 'People asking for / hiring this service' : filter === 'selling' ? 'People offering this service' : 'All posts'}
              </span>
            </div>
            {filteredLeads.length === 0 ? (
              <div className="text-center py-12 text-sm text-slate-500">
                No {filter === 'buying' ? 'buyer' : filter === 'selling' ? 'seller' : ''} posts{location.trim() ? ' matching this location' : ' in this batch'}. Try another filter{location.trim() ? ' or clear the location' : ''}.
              </div>
            ) : (
              filteredLeads.map(lead => (
                <LeadCard key={lead.id} lead={lead} onSave={() => onSaveLead(lead)} isSaved={sessionSavedIds.has(lead.id)} />
              ))
            )}
          </div>
        )}
